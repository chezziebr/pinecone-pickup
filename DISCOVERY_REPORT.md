...
|---|---|---|
| Customer lands on marketing page | **Working** | `app/page.tsx` composes Nav, Hero, HowItWorks, Pricing, BookingForm, Footer. (Testimonials section removed 2026-04-27 cluster 3.) |
...
**Read paths only.** `/api/admin/stats` computes an average rating from this table; `/api/admin/customers/[email]` shows reviews per customer. The write paths (`/api/review` endpoint, `/review` page, `sendReviewRequest` cron) were removed in cluster 2 (2026-04-27); the marketing-page Testimonials section that hardcoded fake customers was removed in cluster 3 (2026-04-27). Reviews table is empty as of 2026-04-27 and frozen — no path writes new rows.
...
4. **Testimonials section removed** — RESOLVED 2026-04-27 (cluster 3). `components/Testimonials.tsx` (which hardcoded fake customer testimonials with specific names and neighborhoods) was deleted; the import + render were removed from `app/page.tsx`. The `/api/review` write path was already removed in cluster 2. Reviews table is empty and frozen. Read paths in `/api/admin/stats` and `/api/admin/customers/[email]` retained — they degrade gracefully on empty data.
...
- **Testimonials section was removed** (cluster 3, 2026-04-27) — the hardcoded fake-customer block is gone; the customer-facing review-collection path was removed in cluster 2; reviews table is empty. Re-introduce a section only when a curated source of real testimonials exists.
