...
| 2026-04-27 | Session 5 — migration 001 reconciliation (commit c2487b2). Migration 006 applies the parts of migration 001 that never landed: `check_price_positive` and `check_valid_lot_size` on `bookings`, `unique_review_per_booking` on `reviews`, the `validate_booking_data` and `validate_review_data` trigger functions and triggers, the `audit_logs` table, the `audit_trigger` function, and `audit_bookings_trigger`. Deliberately omitted: `CHECK (scheduled_date >= CURRENT_DATE)` on `bookings` and the matching `IF NEW.scheduled_date < CURRENT_DATE` branch inside `validate_booking_data()` — both would block the review-request cron's `confirmed → completed` UPDATE on past-dated bookings; API-layer `validateFutureDate` enforces the actual business rule (future date at create time). Session 4's "migration 001 partially applied" deferred-decisions item moved to resolved (Known latent issues #5) with a sharper framing: 001's *named* constraints never applied at all — the pre-006 UI-named CHECKs were Supabase-UI-generated before any migrations existed, and 001 likely failed at first `ALTER TABLE ADD CONSTRAINT`. Audit-trail caveat documented: `audit_trigger.changed_by` will be `NULL` on every row under service-role connections (no JWT claims), which is acceptable while `audit_logs` remains unread but matters if a reader is ever built. DISCOVERY §3 correction appended in companion commit. |
